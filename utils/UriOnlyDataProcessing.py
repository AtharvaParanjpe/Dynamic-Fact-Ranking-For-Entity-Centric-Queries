...
## Data Processing for the URI only Dataset including the URI objects
...
class DataPreprocess:
    ...
    def __init__(self,pathToFile):
        ...
        dataFrame = self.retainUriOnly(dataFrame)
    ...
    def retainUriOnly(self,df):
        for index, row in df.iterrows():
            if(
                ('dbp' in row['obj']) or ('dbpedia' in row['obj']) or ('dbo' in row['obj'])):
                    continue
            else:
                df = df.drop(index)
        return df
    ...
    def convertToExcel(self,df):
        df.to_csv("../data/URI_with_targets.csv",index = False)
